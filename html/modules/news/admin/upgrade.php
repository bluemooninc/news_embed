...
if (is_object($xoopsUser) && $xoopsUser->isAdmin($xoopsModule->mid()))
{
	...
	if(!TableExists($xoopsDB->prefix('stories_files')))
	{
		...
		if (!$xoopsDB->queryF($sql)) {
	    	echo '<br />' . _AD_NEWS_UPGRADEFAILED.' '._AD_NEWS_UPGRADEFAILED1;
	    	$errors++;
		}
	}
	...
	if(!TableExists($xoopsDB->prefix('stories_votedata')))
	{
		...
		if (!$xoopsDB->queryF($sql)) {
	    	echo '<br />' .  _AD_NEWS_UPGRADEFAILED.' '._AD_NEWS_UPGRADEFAILED3;
	    	$errors++;
		}
	}
	...
	if($errors) {
		echo "<H1>" . _AD_NEWS_UPGRADEFAILED . "</H1>";
		echo "<br />" . _AD_NEWS_UPGRADEFAILED0;
	} else {
		echo _AD_NEWS_UPGRADECOMPLETE." - <a href='".XOOPS_URL."/modules/system/admin.php?fct=modulesadmin&op=update&module=news'>"._AD_NEWS_UPDATEMODULE."</a>";
	}
} else {
	printf("<H2>%s</H2>\n",_AD_NEWS_UPGR_ACCESS_ERROR);
}
xoops_cp_footer();
?>
