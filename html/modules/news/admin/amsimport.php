...
if (is_object($xoopsUser) && $xoopsUser->isAdmin($xoopsModule->mid())) {
	if(!isset($_POST['go']) ) {
			...
			while ( $article = $db->fetchArray($result) ) {
				...
				if($use_extlinks) {
					...
					while ( $link = $db->fetchArray($result7) ) {
						if(trim($links)=='') {
							$links="\n\n"._AMS_MD_RELATEDARTICLES."\n\n";
						}
						$links .= _AMS_MD_EXTERNALLINK.' [url='.$link['link_link'].']'.$link['link_title'].'[/url]'."\n";
					}
				}
				...
				if($use_forum && $one_amstopic['forum_id']!=0) {
					$forum = "\n\n".'[url='.XOOPS_URL.'/modules/newbb/viewforum.php?forum='.$one_amstopic['forum_id'].']'._AMS_AD_LINKEDFORUM.'[/url]'."\n";
				}
				...
				// The comments
				$comments =& $comment_handler->getByStoryId($ams_mid, $ams_newsid, 'ASC');
				if(is_array($comments) && count($comments)>0) {
					foreach($comments as $onecomment) {
						$onecomment->setNew();
						$onecomment->setVar('com_modid',$news_mid);
						$onecomment->setVar('com_storyid',$news_newsid);
				...
				//$notifications =& $notification_handler->getByStoryId($ams_mid, $ams_newsid, 'ASC');
	        	$criteria = new CriteriaCompo(new Criteria('not_modid', $ams_mid));
    	    	$criteria->add(new Criteria('not_storyid', $ams_newsid));
				...
				if(is_array($notifications) && count($notifications)>0) {
					foreach($notifications as $onenotification) {
						...
						$onenotification->setVar('not_storyid',$news_newsid);
		...
		// Finally, import all the globals notifications
       	$criteria = new CriteriaCompo(new Criteria('not_modid', $ams_mid));
    	$criteria->add(new Criteria('not_topics', 'global'));
       	$criteria->setOrder('ASC');
   		$notifications = $notification_handler->getObjects($criteria);
		if(is_array($notifications) && count($notifications)>0) {
			foreach($notifications as $onenotification) {
