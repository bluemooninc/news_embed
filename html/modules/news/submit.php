...
switch ($op) {
    case 'edit':
		$xoopsTpl->assign('lang_editarticle', _AD_EDITARTICLE); // by makinosuke @ 2009.1.5
		$xoopsTpl->assign('lang_pagetitle', _AD_EDITARTICLE); // by makinosuke @ 2009.1.10
...
//        echo "<h4>" . _AD_EDITARTICLE . "</h4>"; // by makinosuke @ 2009.1.5
...
//        echo"</td></tr></table>"; // by makinosuke @ 2009.1.5
	...
	case 'preview':
		$xoopsTpl->assign('lang_pagetitle', _AD_EDITARTICLE); // by makinosuke @ 2009.1.10
		$xoopsTpl->assign('lang_pagesubtitle', _AD_PREVIEW); // by makinosuke @ 2009.1.10
...
//		themecenterposts($p_title, $p_hometext);//by makinosuke @ 2009.1.9
	...
	case 'post':
						...
						*/
						...
						if ($uploader->fetchMedia($_POST['xoops_upload_file'][0])) {
							if ($uploader->upload()) {
								...
								if(!$sfiles->store()) {
									echo _AD_UPLOAD_DBERROR_SAVE;
								}
							} else {
								echo _AD_UPLOAD_ERROR. ' ' . $uploader->getErrors() . $_FILES[$_POST['xoops_upload_file'][0]]['type'];
							}
						} else {
		...
		$returnside=intval($_POST['returnside']);
		if(!$returnside) {
			redirect_header(XOOPS_URL."/modules/news/index.php",2,_MD_THANKS);
			exit();
		} else {
			redirect_header(XOOPS_URL."/modules/news/admin/index.php?op=newarticle",2,_MD_THANKS);
			exit();
		}
		break;

	case 'form':
		$xoopsTpl->assign('lang_pagetitle', _AD_POSTNEWARTICLE); // by makinosuke @ 2009.1.10
		$xt = new NewsTopic();
		$title = '';
		$hometext = '';
		$noname = 0;
