...
$story['mail_link'] = 'mailto:?subject='.sprintf(_MD_INTARTICLE,$xoopsConfig['sitename']).'&amp;body='.sprintf(_MD_INTARTFOUND, $xoopsConfig['sitename']).':  '.XOOPS_URL.'/modules/news/article.php?storyid='.$article->storyid();
$xoopsTpl->assign('lang_printerpage', _MD_PRINTERFRIENDLY);
$xoopsTpl->assign('lang_sendstory', _MD_SENDSTORY);
$xoopsTpl->assign('lang_pdfstory', _MD_MAKEPDF);
$xoopsTpl->assign('lang_on', _ON);
$xoopsTpl->assign('lang_postedby', _POSTEDBY);
$xoopsTpl->assign('lang_reads', _READS);
$xoopsTpl->assign('mail_link', 'mailto:?subject='.sprintf(_MD_INTARTICLE,$xoopsConfig['sitename']).'&amp;body='.sprintf(_MD_INTARTFOUND, $xoopsConfig['sitename']).':  '.XOOPS_URL.'/modules/news/article.php?storyid='.$article->storyid());

$xoopsTpl->assign('lang_attached_files',_MD_ATTACHEDFILES);
$sfiles = new sFiles();
$filesarr = Array();
$newsfiles = Array();
...
if(getmoduleoption('newsbythisauthor')) {
	$xoopsTpl->assign('news_by_the_same_author_link',sprintf("<a href='%s?uid=%d'>%s</a>",XOOPS_URL.'/modules/news/newsbythisauthor.php',$article->uid(),_MD_NEWSSAMEAUTHORLINK));
}
...
if (getmoduleoption('showsummarytable')) {
	...
	$xoopsTpl->assign('lang_other_story',_MD_OTHER_ARTICLES);
	$count=0;
...
if (getmoduleoption('showprevnextlink')) {
	...
	if(is_array($sarray)) {
   	...
   	$xoopsTpl->assign('lang_previous_story',_MD_PREVIOUS_ARTICLE);
   	$xoopsTpl->assign('lang_next_story',_MD_NEXT_ARTICLE);
   	unset($tmparticle);
} else {
...
if (getmoduleoption('ratenews')) {
	...
	if ($article->votes == 1) {
		$story['votes'] = _MD_ONEVOTE;
	} else {
		$story['votes'] = sprintf(_MD_NUMVOTES,$article->votes);
	}
} else {
