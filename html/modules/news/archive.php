...
$xoopsTpl->assign('xoops_pagetitle', $myts->htmlSpecialChars(_MD_NEWSARCHIVES)." ".$pgtitle . ' | ' . $myts->htmlSpecialChars($xoopsModule->name()));//by makinosuke @2008/3/25
...
if ($fromyear != 0 && $frommonth != 0) {
	...
	$xoopsTpl->assign('lang_articles', _MD_ARTICLES);
	...
	$xoopsTpl->assign('lang_actions', _MD_ACTIONS);
	...
	$xoopsTpl->assign('lang_storytotal', sprintf(_MD_THEREAREINTOTAL, $count));
} else {
    $xoopsTpl->assign('show_articles', false);
}

$xoopsTpl->assign('lang_newsarchives', _MD_NEWSARCHIVES);
