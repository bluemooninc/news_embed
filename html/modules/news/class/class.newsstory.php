	...
	function prepare2show($filescount)
	{
		...
		if ($xoopsModuleConfig['ratenews']) {
			...
			if ($this->votes == 1) {
				...
				$story['votes'] = sprintf(_MD_NUMVOTES,$this->votes);
			}
		}
	    ...
	    $morelink = '';
	    if ( $fullcount > 1 ) {
	        $morelink .= '<a href="'.XOOPS_URL.'/modules/news/app/article.php?storyid='.$this->storyid().'';
	        $morelink .= '">'._MD_READMORE.'</a>';
//        	$morelink .= ' | '.sprintf(_MD_BYTESMORE,$totalcount); // by makinosuke
	    ...
	    if (XOOPS_COMMENT_APPROVENONE != $xoopsModuleConfig['com_rule']) {
	        ...
	        if ( $ccount == 0 ) {
	            $morelink .= '">'._MD_COMMENTS.'</a>';
	        } else {
	            if ( $fullcount < 1 ) {
	                if ( $ccount == 1 ) {
	                    $morelink .= '">'._MD_READMORE.'</a> | '.$morelink2.'">'._MD_ONECOMMENT.'</a>';
	                } else {
	                    $morelink .= '">'._MD_READMORE.'</a> | '.$morelink2.'">';
	                    $morelink .= sprintf(_MD_NUMCOMMENTS, $ccount);
	                    $morelink .= '</a>';
	                }
	            } else {
	                if ( $ccount == 1 ) {
	                    $morelink .= '">'._MD_ONECOMMENT.'</a>';
	                } else {
	                    $morelink .= '">';
	                    $morelink .= sprintf(_MD_NUMCOMMENTS, $ccount);
	                    $morelink .= '</a>';
	    ...
	    $story['mail_link'] = 'mailto:?subject='.sprintf(_MD_INTARTICLE,$xoopsConfig['sitename']).'&amp;body='.sprintf(_MD_INTARTFOUND, $xoopsConfig['sitename']).':  '.XOOPS_URL.'/modules/news/article.php?storyid='.$this->storyid();
	    $story['imglink'] = '';
	    $story['align'] = '';
		...
		if($filescount>0) {
			...
			$story['attached_link']="<a href='".XOOPS_URL.'/modules/news/article.php?storyid='.$this->storyid()."' title='"._MD_ATTACHEDLIB."'><img src='".XOOPS_URL.'/modules/news/images/attach.gif'."' title='"._MD_ATTACHEDLIB."'></a>";
		} else {
