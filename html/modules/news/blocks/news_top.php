		...
		if($currenttab==0 && $usespotlight) {	// Spotlight or not ?
			...
			if(strlen(xoops_trim($tmpstory->bodytext()))>0) {
       		...
       		if($tmpstory->votes()==1) {
				...
				$spotlight['number_votes']=sprintf(_MD_NUMVOTES,$tmpstory->votes());
			}

       		$spotlight['votes_with_text']=sprintf(_MD_NUMVOTES,$tmpstory->votes());
			...
			if($tabscount>0) {
    			...
    			$smallheader[]=sprintf("%u %s",$stats['count'],_MD_ARTICLES);
				...
				if(count($stories)>0) {
    				foreach ($stories as $key => $story) {
		            	...
		            	if($story->votes()==1) {
							$news['number_votes']=_MD_ONEVOTE;
						} else {
							$news['number_votes']=sprintf(_MD_NUMVOTES,$story->votes());
						}
						if($infotips>0) {
							$news['infotips'] = ' title="'.make_infotips($story->hometext()).'"';
						} else {
